...
const Login = ({ users, onAcc }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const userAcc = users.filter(
      (user) => user.name == name && user.pass == pass
    ).length;
    if (adminUserName == name && adminPassword == pass)
      return setName(""), setPass(""), navigate("/admin");
    else if (userAcc !== 0 || userAcc) {
      const item = {
        id: Date.now(),
        name: name,
        pass: pass,
      ...
      return setName(""), setPass(""), navigate("/register");
  };
  return (
    <div>
